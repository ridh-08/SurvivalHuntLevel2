...
let score = 0;
let totalClicks = 0;
let maxClicks = 50; // Set the maximum number of clicks allowed
let startTime;
let elapsedTime = 0;
...
function draw() {
    if (gameRunning) {
        ...
        text('Total Clicks: ' + totalClicks, 20, 100); // Display the total clicks
        text('Maximum Clicks: ' + maxClicks, 20, 70);
        ...
        text('Total Clicks = ' + totalClicks, width / 2, 300);
        text('Maximum Clicks = ' + maxClicks, width/2, 250);
        textSize(24);
        textWrap(WORD);
        textAlign(LEFT);
        textSize(12);
...
function mouseClicked() {
    if (totalClicks >= maxClicks) {
        gameRunning= false; // If maximum clicks are reached, do nothing
    }
